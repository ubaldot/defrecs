...
REM "%openocd_exe%" "-f" "%probes_path%\%probe%" "-f" "%boards_path%\%board%" "-c" "stm32f4x.cpu configure -event gdb-detach { resume }"
"%openocd_exe%" "-f" "%probes_path%\%probe%" "-f" "%boards_path%\%board%" "-d3"
