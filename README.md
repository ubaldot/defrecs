...
think in terms or "connecting blocks" but instead of generating code from
...
therein contained describe the application and you can see how the components
are connected. The theoretical part that serve as basis for all the examples
is discussed in the reminder of this `README` file.
...
shipped with CubeMX. It takes a bit of application because components actually
use FreeRTOS API.
...
4. A component should be scheduled only in one periodic task. Avoid calling
   the same component from different periodic task, However, an already
