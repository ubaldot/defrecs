...
The folders starting with a Capital letter are generated by CubeMX.
Nevertheless, some code contained in these generated files must be updated. To
see what has been updated check the code in between the comments
`/* USER CODE BEGIN ... */` and `/* USER CODE END ... */`, for example see how
a Callback function in `./Core/Src/stm32f4xx_it.c` has been added.

However, the most important folders are **platform** and **application**
folders. They contain all the components. Note how each component specifies
publish and subscribe functions for reading inputs and writing outputs, how
they have an init and a step function and how mutex are used. Also, note how
the structure of both publish and subscribe functions is the same: you have a
mutex and you use a `memcpy` depending on the size of the signal you want to
publish. However, the publish function is defined as static and it does not
...
You also have a `.ioc` file to be opened with CubeMX and a `Makefile` firstly
generated by CubeMX and then edited to add e.g. additional source and header
files, some targets, etc.
